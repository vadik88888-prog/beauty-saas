...
30. **После миграции, добавляющей колонку в `tenant_ai_settings`, — обязательно перезагрузить схему PostgREST.** `loadTenantConfig` делает один SELECT ~10 колонок с `.single()`; если хоть одной колонки нет в кэше PostgREST, весь запрос падает, ошибка глотается молча, и весь конфиг (движок, модель, температура, имя) уходит в дефолт — `bookingEngine` становится `legacy` без единой строки в логах. После любой ручной миграции с `ADD COLUMN` на `tenant_ai_settings` выполнить в Supabase SQL Editor: `NOTIFY pgrst, 'reload schema';`. **Тех-долг:** `loadTenantConfig` должен логировать `settingsRes.error`, а не глотать его — сейчас ошибка невидима.
