...
export default async function ClientProfilePage({
  ...
  // Status flags — same thresholds as /clients list and dashboard
  ...
  const avgCheck = client.total_visits > 0 && client.total_spent > 0
    ...
    : null
  const currency = visits.find(v => v.service?.currency)?.service?.currency ?? 'BYN'
  const firstVisit = visits.length > 0 ? visits[visits.length - 1].starts_at : null
  ...
  if (completedVisits.length >= 2) {
    ...
    for (let i = 1; i < completedVisits.length; i++) {
      totalInterval +=
        (new Date(completedVisits[i].starts_at).getTime() -
         new Date(completedVisits[i - 1].starts_at).getTime()) / 86_400_000
    }
    const avgIntervalDays = Math.round(totalInterval / (completedVisits.length - 1))
    ...
    <>
      ...
      <div style={{ padding: '20px 20px 48px', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <PageHeader
          ...
          action={
            <Link href="/chats">
              <button className="sera-btn sera-btn--secondary sera-btn--sm" style={{ gap: 6 }}>
        ...
        <div className="profile-layout">
          ...
          <aside className="profile-sidebar">
            ...
            {/* SERA rhythm block — one SeraOrb on this page (economy rule) */}
            <div className="sera-card" style={{ padding: 'var(--space-5)' }}>
              ...
              {noVisits ? (
                ...
                <div>
                  {visits.map((visit, i) => {
                    ...
                    const status = (visit.status as AppointmentStatus) in STATUS_LABELS
                      ? visit.status as AppointmentStatus
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          padding: '11px 16px',
                          borderBottom: i < visits.length - 1 ? '1px solid var(--line-soft)' : 'none',
                        }}
                      >
                        {/* Date */}
                        <div style={{ flexShrink: 0, width: 88 }}>
                          ...
                          <p style={{ fontSize: 11, color: 'var(--muted)', margin: '1px 0 0', fontFamily: 'var(--font-mono)' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 2 }}>
                            ...
                            {isAi && (
                              <Sparkles size={10} style={{ color: 'var(--gold)', flexShrink: 0 }} />
                            )}
                          </div>
