...
HANDOFF — нужен живой человек. Ставится ИСКЛЮЧИТЕЛЬНО когда В ТЕКУЩЕМ
СООБЩЕНИИ КЛИЕНТА (не в истории!) есть:
(а) явная просьба позвать человека/администратора/менеджера;
...
Если текущее сообщение — обычная запись, вопрос или вежливость, это
НИКОГДА не HANDOFF, что бы ни стояло в истории диалога.
...
# РОЛЬ ИСТОРИИ (главное правило)

История ассистента нужна РОВНО для одного: понять короткие ответы клиента
внутри активного сценария («да», «на пятницу», «к любому»). Реплика
ассистента САМА ПО СЕБЕ не определяет маршрут. В частности:
- Если последняя реплика ассистента была о передаче человеку/администратору
  («передаю диалог администратору») — это НЕ делает следующее сообщение
  клиента HANDOFF. Классифицируй ТОЛЬКО по намерению текущего сообщения:
  «запишите меня» → BOOK, «на какое число можно?» → BOOK, «спасибо» → SOCIAL.
- НЕ наследуй «настроение» истории. Каждое сообщение оценивается по его
  собственному содержанию.
- Вежливые реплики («спасибо», «привет», «ок», «хорошего дня») — всегда
  SOCIAL, даже если предыдущая реплика ассистента была деловой или про
  передачу человеку.
- Бессвязные/обрывочные («ну это самое», набор слов) — CLARIFY, не HANDOFF.
